...
public class Algoritmo {  
    ...
    /**
     * agregamos los resultados de mayor longitud
     * En caso de que hayan varios del mismo tamano
     * todas seran agregadas a una sola var de tipo String
     */
    protected String devolverResultado() {     
        String res = "";
        ...
        for(int i = 0; i < resultado.size(); i++){
            if(resultado.get(i).size() == tam){//Si es del tamano max, se imprimira                
                    for(int ii = 0; ii < resultado.get(i).size(); ii++){
                        res = res + resultado.get(i).get(ii);//Agregamos los caracteres al String de resultado
                                            }
